...
const CLIENT_EVENTS = new Set(["site_visited", "product_started", "generation_completed", "pdf_downloaded"]);
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (event !== "site_visited" && !isTelemetryProduct(product)) {
    ...
    logTelemetry(`pmm_${event}` as "pmm_site_visited" | "pmm_product_started" | "pmm_generation_completed" | "pmm_pdf_downloaded", {
