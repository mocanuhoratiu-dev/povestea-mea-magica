const MATERIAL_TERMS = /\b(poveste|povestea|album|albumul|scut|scutul|trus[aă]|trusa|ritual|activitat(?:e|ea)|material|produs)\b/i;
...
export function wantsLumiMaterialRecommendation(message: string, previousModelMessage = "") {
  ...
  return SHORT_CONFIRMATION.test(cleanMessage)
    && /\b(recomand|aleg|material|poveste|album|scut|trus[aă])\b/i.test(cleanPreviousMessage);
}
