...
export function readAlbumOutput(value: unknown): AlbumOrderOutput | null {
  ...
  return {
    ...
    budget: isRecord(value.budget) ? {
      ...
      maxImageCalls: Math.max(1, Number(value.budget.maxImageCalls) || 36),
      maxQualityCalls: Math.max(0, Number(value.budget.maxQualityCalls) || 36),
      estimatedCostMicros: Math.max(0, Number(value.budget.estimatedCostMicros) || 0),
      maxEstimatedCostMicros: Math.max(100_000, Number(value.budget.maxEstimatedCostMicros) || 2_200_000),
    } : {
      ...
      maxEstimatedCostMicros: 2_200_000,
    },
    progress: {
